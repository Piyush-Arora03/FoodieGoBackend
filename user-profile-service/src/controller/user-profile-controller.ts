...
const getUserProfile = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userProfile: Profile | null = await userProfileService.getUserProfile(userId);
...
const createOrUpdateUserProfile = async (req: Request, res: Response) => {
    try {
        const userId: string = req.user!.id;
        const userProfileData: UserProfilePayload = req.body;
        const userProfile: Profile | null = await userProfileService.createOrUpdateUserProfile(userId, userProfileData);
        res.status(STATUS_CODE.OK).json({
            data: userProfile,
            success: true,
